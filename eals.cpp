...
public:
  ...
  void loadData() {
    ...
    int train_user = coodata.row;
    coodata.read(fname_val);
    coodata.organize(train_user);
    val_user = coodata.row - train_user;
  ...
  void train() {
    ...
    for(int u=0;u<P.rows();u++) {
      for(int f=0;f<num_factor;f++) {
        ...
        for(int i_ind=0;i_ind<col_num[u];i_ind++) {
          qif = Q(rui_hat[u][i_ind].ind, f);
          rui_hat[u][i_ind].val = P(u, f) * qif + rui_hat_f[i_ind]; 
        }
      }
    }
    for(int u=0;u<P.rows();u++) 
    ...
    for(int i=0;i<Q.rows();i++) {
      for(int f=0;f<num_factor;f++) {
        ...
        for(int u_ind=0;u_ind<row_num[i];u_ind++) {
          puf = P(riu_hat[i][u_ind].ind, f);
          riu_hat[i][u_ind].val = riu_hat_f[u_ind] + Q(i, f) * puf;
        }
      }
    }
    for(int i=0;i<Q.rows();i++) 
